...
def setup_database():
    ...
    try:
        ...
        print("4. Starting ingestion (batching 100 rows in one transaction)...")
        text_batch = []
        vec_batch = []

        # Keep the previous database intact if streaming or embedding fails.
        with db:
            db.execute("BEGIN")
            ...
            for i, row in enumerate(ds):
                ...
                if len(text_batch) >= 100:
                    db.executemany(
                        "INSERT INTO medquad (id, question, answer) VALUES (?, ?, ?)",
                        text_batch,
                    )
                    db.executemany(
                        "INSERT INTO vec_medquad (rowid, embedding) VALUES (?, ?)",
                        vec_batch,
                    )
                    text_batch = []
                    vec_batch = []
                    print(f"   ...Prepared {row_id} rows")
